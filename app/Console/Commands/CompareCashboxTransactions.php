...
class CompareCashboxTransactions extends Command
{
    ...
    protected function compareCashboxSummary($remoteCashboxes): void
    {
        ...
        $this->table(
            ['ID', 'Cashbox', 'Local Balance', 'Production Balance', 'Difference', 'Status'],
            $rows
        );
    }
    ...
    protected function compareTransactions($localTransactions, $remoteTransactions): void
    {
        ...
        if (count($dateMismatches) > 0) {
            ...
            $this->table(
                ['ID', 'CB', 'Description', 'Amount', 'L.Date', 'P.Date', 'L.Time', 'P.Time', 'L.Balance', 'P.Balance', 'Diff In'],
                $rows
            );
    ...
    protected function renderLocalTable(array $items): void
    {
        ...
        $this->table(
            ['ID', 'CB', 'Type', 'Amount', 'Description', 'Date', 'Created At', 'Ref', 'Ref ID'],
            $rows
        );
    }
