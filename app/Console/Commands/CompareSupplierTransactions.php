...
class CompareSupplierTransactions extends Command
{
    protected $signature = 'sync:compare-supplier-transactions
        {--device= : Device ID to use for API auth}
        {--supplier= : Compare specific supplier ID only}
        {--date-from= : Filter from date (Y-m-d)}
        {--date-to= : Filter to date (Y-m-d)}';

    protected $description = 'Compare local supplier transactions with production server and show differences';
    ...
    public function handle(): int
    {
        ...
        $localQuery = SupplierTransaction::query();
        $supplierId = $this->option('supplier');

        if ($supplierId) {
            $localQuery->where('supplier_id', $supplierId);
        }
        ...
        $this->compareSupplierSummary($remoteSuppliers);
        $this->newLine();
    ...
    protected function fetchRemoteTransactions(): ?array
    {
        ...
        if ($this->option('supplier')) {
            $params['supplier_id'] = $this->option('supplier');
        }
        if ($this->option('date-from')) {
        ...
        try {
            ...
            ])->timeout(60)->get("{$this->serverUrl}/api/v1/sync/compare-supplier-transactions", $params);
    ...
    protected function compareSupplierSummary($remoteSuppliers): void
    {
        $this->warn('=== Supplier Balances ===');

        $supplierQuery = Supplier::query();
        if ($this->option('supplier')) {
            $supplierQuery->where('id', $this->option('supplier'));
        }
        $localSuppliers = $supplierQuery->get();
        ...
        $this->table(
            ['ID', 'Supplier', 'Local Balance', 'Production Balance', 'Difference', 'Status'],
            $rows
        );
    }
    ...
    protected function compareTransactions($localTransactions, $remoteTransactions): void
    {
        ...
        if (count($dateMismatches) > 0) {
            ...
            $this->table(
                ['ID', 'Supp', 'Description', 'Amount', 'L.Date', 'P.Date', 'L.Time', 'P.Time', 'L.Balance', 'P.Balance', 'Diff In'],
                $rows
            );
    ...
    protected function renderLocalTable(array $items): void
    {
        ...
        $this->table(
            ['ID', 'Supp', 'Type', 'Amount', 'Description', 'Date', 'Created At', 'Ref', 'Ref ID'],
            $rows
        );
    }
