...
<body>
    <div class="print-container">
        ...
        <div class="header">
            ...
            <div class="subtitle">أرصدة ومشتريات الموردين</div>
            <div class="date-info">
        ...
        <div class="summary-grid">
            <div class="summary-box">
                ...
                <div class="value">{{ number_format($summary['total_suppliers']) }}</div>
            </div>
            <div class="summary-box highlight">
                <div class="label">إجمالي الأرصدة</div>
                <div class="value negative">{{ number_format($summary['total_balance'], 2) }}</div>
            </div>
            <div class="summary-box">
                <div class="label">إجمالي المشتريات</div>
                <div class="value warning">{{ number_format($summary['total_purchases'], 2) }}</div>
            </div>
            <div class="summary-box">
        ...
        <div class="two-columns">
            @if(!empty($reportData['suppliers']))
            <div class="section">
                <div class="section-title">قائمة الموردين ({{ count($reportData['suppliers']) }} مورد)</div>
                <table>
                    <thead>
                        <tr>
                            <th>#</th>
                    ...
                    <tbody>
                        @foreach($reportData['suppliers'] as $index => $row)
                        <tr>
                            <td class="text-center">{{ $index + 1 }}</td>
                            <td>{{ $row['name'] }}</td>
                            <td>{{ $row['phone'] ?? '-' }}</td>
                            <td class="text-left {{ $row['current_balance'] != 0 ? 'text-negative' : '' }}">{{ number_format($row['current_balance'], 2) }}</td>
                            <td class="text-center">{{ number_format($row['purchase_count']) }}</td>
                            <td class="text-left text-warning">{{ number_format($row['total_purchases'], 2) }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr>
                            ...
                            <td class="text-left text-negative">{{ number_format($summary['total_balance'], 2) }}</td>
                            <td></td>
                            <td class="text-left text-warning">{{ number_format($summary['total_purchases'], 2) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="section">
                <div class="section-title">أكبر الموردين</div>
                <table>
                    <thead>
                        <tr>
                            <th>الاسم</th>
                    ...
                    <tbody>
                        @foreach($reportData['top_suppliers'] as $row)
                        <tr>
                            <td>{{ $row['name'] }}</td>
                            <td class="text-left text-warning">{{ number_format($row['total_purchases'], 2) }}</td>
                        </tr>
