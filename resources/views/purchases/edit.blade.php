...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('purchases.show', $purchase) }}">فاتورة #{{ $purchase->id }}</a></li>
...
<div class="page-header">
    <h4>تعديل فاتورة مشتريات #{{ $purchase->id }}</h4>
</div>
            ...
            <div class="purchase-card">
                ...
                <div class="purchase-card-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <div class="supplier-balance {{ $purchase->supplier_id ? 'show' : '' }}" id="supplierBalanceBox" style="background-color: #0d6efd;">
                                <div class="supplier-balance-label" style="color: #fff;">الرصيد الحالي</div>
                                <div class="supplier-balance-value" id="supplierBalanceValue" style="color: #fff;">{{ number_format($purchase->supplier->current_balance ?? 0, 2) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" id="invoice_number" name="invoice_number"
                                   value="{{ $purchase->invoice_number }}" placeholder="اختياري">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control" id="notes" name="notes" rows="2"
                                      placeholder="ملاحظات إضافية على الفاتورة...">{{ $purchase->notes }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="purchase-card-body">
                    ...
                    <div class="discount-tax-group">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <input type="number" class="form-control" id="discount_value" name="discount_value"
                                   ...
                                   {{ !$purchase->discount_type ? 'disabled' : '' }}>
                        </div>
                    </div>
                    ...
                    <div class="discount-tax-group">
                        ...
                        <input type="number" class="form-control form-control-sm" id="tax_rate" name="tax_rate"
                               value="{{ $purchase->tax_rate }}" min="0" max="100" step="0.01" placeholder="0">
                    </div>
                    ...
                    <div class="action-buttons">
                        ...
                        <a href="{{ route('purchases.show', $purchase) }}" class="btn btn-outline-danger">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Define global functions first (before loading existing items)
    ...
    @php
        $existingItemsData = $purchase->items->map(function($item) {
            return [
                ...
                'units' => $item->product->productUnits->map(function($pu) {
                    return [
                        'id' => $pu->id,
                        'name' => $pu->unit?->name ?? '-',
    ...
    function addProductToItems(product) {
        ...
        const baseUnit = product.base_unit;
        const unitPrice = parseFloat(baseUnit?.cost_price) || 0;
        const item = {
            index: itemIndex,
    ...
    function renderItemRow(item) {
        ...
        if (item.units && item.units.length > 0) {
            item.units.forEach(unit => {
                ...
                const costPrice = parseFloat(unit.cost_price) || 0;
                unitsOptions += `<option value="${unit.id}" data-multiplier="${unit.multiplier}" data-cost="${costPrice}" ${selected}>
                    ${unit.name}
                </option>`;
            });
        }
    ...
    const saveAndApproveBtn = document.getElementById('saveAndApproveBtn');
    const saveAsDraftBtn = document.getElementById('saveAsDraftBtn');

    if (saveAndApproveBtn) {
        saveAndApproveBtn.addEventListener('click', () => submitForm(false));
    }
    if (saveAsDraftBtn) {
        saveAsDraftBtn.addEventListener('click', () => submitForm(true));
    }
